...
void main() {
  ...
  float height = snoise(vec2(uv.x * 4.0 + uTime * 0.1, uTime * 0.2)) * 0.5 * uAmplitude;
  height = exp(height);
  height = (uv.y * 2.2 - height + 0.2);
  float intensity = 1.5 * height;
...
export const Aurora = (props: AuroraProps) => {
    // Colors: White -> Black -> White (Symmetrical for full coverage)
    const {
    ...
    return (
        <div className="fixed inset-0 z-[-2] pointer-events-none opacity-[0.5] mix-blend-screen overflow-hidden">
