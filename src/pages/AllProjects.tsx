                ...
                >
                    <AnimatePresence mode="popLayout">
                        {filteredProjects.map((project) => (
                            ...
                            >
                                {/* Image */}
                                <div className={`h-64 overflow-hidden relative ${['fenrir', 'orca'].includes(project.id) ? 'bg-zinc-950 p-12' : 'bg-surface-hover'}`}>
                                    <img
                                        src={project.image}
                                        alt={project.title}
                                        loading="lazy"
                                        decoding="async"
                                        className={`w-full h-full ${['ciel'].includes(project.id) ? 'object-cover object-top' : ['fenrir', 'orca'].includes(project.id) ? 'object-contain' : 'object-cover'} transition-transform duration-500`}
                                    />
